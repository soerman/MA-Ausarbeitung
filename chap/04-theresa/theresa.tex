...
In this thesis, the software used to cover both of these steps is \textit{PADS xDx Designer} (for schematic capture) and \textit{PADS Layout/Router} (for \gls{pcb} layout design) from \textit{Mentor Graphics} (subsidiary of Siemens).
...
Furthermore, a schematic provides a starting point for automatic placement and routing, i.e. where the components are placed and how they are connected on the physical \gls{pcb}.
The placement and routing is done with the layout design tool.
During the creation of the schematics, the following points have to be considered:

\begin{itemize}
	\item Deciding which components are needed and what the requirements on the performance are. Especially for high-speed components carefully considering specifications like signal rise and fall times, jitter, skew, etc. is crucial to achieve the overall expected performance.
...
\begin{table}[tb]
	...
	\label{tab:fmc_ps}
		...
		\begin{tabularx}{\textwidth}{Xcc}
			\toprule
			\textbf{Voltage}                        & \textbf{Max. current} & \textbf{Max. capacitive load} \\ \midrule
			$V_\text{ADJ}$, \SIrange{0}{3.3}{\volt} &    \SI{4}{\ampere}    &   \SI{1000}{\micro \farad}    \\
			\SI{3.3}{\volt}                         &    \SI{3}{\ampere}    &   \SI{1000}{\micro \farad}    \\
			\SI{12}{\volt}                          &    \SI{1}{\ampere}    &   \SI{1000}{\micro \farad}    \\ \bottomrule
		\end{tabularx}
\end{table}
...
The component has been chosen due to its high bandwidth (> \SI{18}{\GHz}) and low aperture jitter (range of hundreds of femtoseconds). \cite{caselle2013} 
...
\begin{figure}[tb]
	...
	\caption[Delay chip SDI connections]{Diagram of the SDI control pins for the NB6L295 delay chip. The data (\texttt{SDIN}), clock (\texttt{SCLK}) and load (\texttt{SLOAD}) pins are shared by all chips. Only the enable (ENx) signals are routed individually.}
	\label{fig:delay_sdi}
\end{figure}
...
Compared to the \SI{11}{\pico \second} delay step this is very high and therefore not suitable for high temporal accuracy. 
...
\item High heat resistance: Decomposition temperature $T_d$ = \SI{410}{\celsius} %todo celsius is vartheta, Kelvin uses T..., right?
\end{itemize}
...
Formulas to calculate the characteristic impedance are quite complex (see \autoref{app:waveguides}) and not easy to solve.
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=\textwidth, height = 0.5\textwidth]{chap/04-theresa/img/TL/surf/loss}
...
\begin{figure}[tb]
	...
	\caption[DCWG, $Z_\text{diff}$ vs. $w$]{$Z_\text{diff}$ of an edge-coupled differential coplanar waveguidevs. lower trace width $w$, assuming $\varepsilon_r = 3.61$}
...
\begin{figure}[bhbh]
	\centering
	\includegraphics[width=\textwidth, height = 0.5\textwidth]{chap/04-theresa/img/TL/surf_diff/loss}
...
\begin{figure}[tb]
	...
	\caption[SCWG, $Z_\text{diff}$ vs. $s$]{$Z_\text{diff}$ of a symmetrical differential coplanar waveguidevs. trace separation $s$, assuming trace width $w = \SI{88}{\micro \meter}$}
