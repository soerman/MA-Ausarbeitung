...
The ferrite bead mitigates any high-frequency spectral components and therefore protects the analog ground from noise. 
...
\begin{align*}
	...
	\nicefrac{\SI{2}{\ampere}}{(2 \cdot \SI{0.170}{\ampere})}     & > N                                 \\
...
In order to set the output voltage of the regulator to \SI{2.5}{\volt}, the resistor values $R_1 = \SI{12}{\kilo \ohm}$ and $R_2 = \SI{3}{\kilo \ohm}$ are chosen (refer to \autoref{fig:adp1741} and \autoref{eq:ldo}).
...
\begin{figure}[bhbh]
	...
	\caption[Attenuation DCWG for \SI{50}{\mm}]{Attenuation per trace length of the surface differential coplanar waveguide over frequency simulated for trace length of \SI{50}{\mm}}
...
During characterization of the sampling card, the attenuation of these lines should be measured in order to examine whether the simulated values prove true and if some additional measures should be taken into account in order to guarantee proper signal propagation.


\begin{figure}[tb]
	\centering
	\includegraphics[width = \textwidth, height = 0.4\textwidth]{chap/04-theresa/img/TL/off_diff/loss}
	\caption[Attenuation SCWG for \SI{200}{\mm}, $Z_\text{diff}$ vs. $w$]{Attenuation per trace length of the symmetrical differential coplanar waveguide over frequency simulated for trace length of \SI{200}{\mm}}
