...
In this thesis, the software used to cover both of these steps is PADS xDx Designer (for schematic capture) and PADS Layout/Router (for \gls{pcb} layout design) from \textit{Mentor Graphics} (subsidiary of Siemens).
...
Time skew between output channels of these components typically lies in the range of \SI{1.5}{\pico \second}, 20 times smaller than the time skew of the \gls{pll}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 0.85\textwidth]{chap/04-theresa/img/schematic/dac_channel}
...
\begin{itemize}
	...
	\item Placing additional structures to reduce cross-talk and \gls{emi} (e.g. via fences)
...
On the bottom side (\autoref{fig:pcb_back}) of the board, the analog components, i.e. the \glspl{tha} and analog voltage regulators are placed. 
