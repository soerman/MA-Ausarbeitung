...
This time-stretching is accomplished by using chirped optical pulses and dispersion in optical fibers. %TODO ask if you should state 'chromatic dispersion' or similar
...
The time-stretch concept is also very interesting for applications in particle accelerators due to the short timescales involved. %todo maybe omit 'very' and/or replace 'interesting' with some cinnamon
%todo For example in a storage ring, ...
...
Still, commercially available real-time diagnostics systems are limited in memory space. %todo to what is the 'still' refering? can you state numbers here?
...
This creates a problem in applications where a longer observation time (up to hours) is required, e.g. in accelerator applications where the turn-by-turn analysis of the electron bunches is desired in order to study the evolution of the bunch profiles. %todo what is turn-by-turn, is it the same as BBB, maybe add in footnote
...
\section{Objective}
In this thesis, a first demonstrator of a \gls{daq}-system based on the time-stretch concept has been developed. %todo maybe use present
...
This leads to the next challenge: Sampling at several \si{\GHz} with high resolution, implies a large amount of data in the range of Terabits per second. %todo TB/s is a data rate, not total amount of data
...
Furthermore, the system should be compatible with already existing high-speed \gls{daq} frameworks (e.g. based on \gls{pcie}) and can be easily integrated into the system for the user application (e.g. through optical fibers to a distributed instrumentation system). %todo if this is a requirement, use 'should', 'shall' or sth intead of 'can'
...
Electron storage rings, also called synchrotrons, are a potential source of \gls{thz}-radiation. %todo synchtrons != storage rings (but similar) explained in krieger strahlungsquellen book
...
Electrons, which are grouped to ``electron bunches'', are generated with an electron gun and accelerated to relativistic speeds\footnote{almost speed of light} by a pre-accelerator (often a \gls{linac} or a booster ring accelerator). %todo or microtron + booster
...
As illustrated in \autoref{fig:csr}, this results in emission of \gls{csr} the spectrum of which spans from \SI{100}{\GHz} up to \gls{thz}.
...
%todo this is all a bit to short for a paragraph. try using an itemmize?
...
The \textit{Full-Linear Bandwidth} is defined as the frequency at which the \gls{sr} of the \gls{sha} starts to distort the input signal by a specified value. \cite{Lundberg} 
The \gls{sr} is defined as the rate of how much the voltage $v$ changes over time $t$:
\begin{equation}
	\text{SR} = \frac{\text{d}v}{\text{d}t}
\end{equation}

A \gls{sr} of \SI{1}{\volt \per \micro \second} for example means, that the output of the amplifier can not change more than \SI{1}{\volt} over the course of \SI{1}{\micro \second}. \cite{2021Slew} 
