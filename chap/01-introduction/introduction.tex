In many scientific applications and experiments the observation of non-repetitive, statistically rare and very fast occurrences is desired.
As these effects might especially occur on a time range of femtoseconds, real-time measurement systems with fine temporal resolution and long record lengths are necessary.
This imposes high technological challenges on the \glspl{adc} and \glspl{daq} in general.
...
A first demonstration of a concept to overcome these limitations was presented in 1999 by F. Coppinger, A. S. Bhushan and B. Jalali. 
The idea is to stretch the analog signal in time before digitizing it in the converter and in this way relax the demands on the \glspl{adc} performance. 
...
Since then, the time-stretch method was continuously improved and has found usage in many application fields.
For example in biomedical diagnostics, a first demonstration of an artificial intelligence facilitated high-speed phase microscope has been done. 
It uses the time-stretch quantitative phase imaging (TS-QPI), a technique based on the time-stretch concept which enables simultaneous measurement of phase and spatial intensity profiles. This allows label-free classification of cells for cancer diagnostics and drug development. 

The time-stretch concept is also very interesting for applications in particle accelerators.
Relativistic electron bunches interact with their own radiation which leads to spatial microstructures inside of the bunches, also called micro-bunching instabilities. They are a source of intense pulses of \gls{thz} radiation (\gls{csr}) and therefore an important field of study. 
...
The usage of the time-stretch method in different applications has demonstrated the advantages to measure events in femtosecond resolution.
Still, commercially available real-time measurement equipment is limited in memory space. 
The acquisition time of such oscilloscopes at maximal sampling rate lies in the range of milliseconds at best.
It is therefore not possible to measure data continuously over a large range of time. 
...
This challenge was the motivation to design novel ultra-fast acquisition systems based on the photonic time-stretch \gls{adc} concept. 
...
A photonic time-stretch \gls{daq} consists of a photonic part, which covers the time-stretching method and conversion of photons into electrical values with a photo-detector. 
Furthermore, such a system has one or multiple \glspl{adc} that convert the analog values into digital signals.
The digital signals then are processed in a computing unit and when necessary broadcast to other units, if the system is integrated in a cluster of measurement systems.
 

\section{Objective}
In this thesis, a first demonstrator of a \gls{daq}-system based on the time-stretch concept is developed.
This system, which is called \gls{theresa} system, should enable high-speed measurements of ultrafast events with a resolution in the range of femtoseconds.
In order to achieve such high resolution, the time-stretch technique should be used in order to stretch the input signal in the range of pico- to nanoseconds.
The input signal should be continuously sampled by high-speed \glspl{adc} with a temporal resolution which can be flexibly defined by the user.
To sample the signal, the \glspl{adc} need to have a sampling rate in the order of several GHz.
As the \glspl{adc} also need a high resolution, i.e. > 10 bits, this leads to the next challenge: Sampling at several GHz with high resolution implies a large amount of data, touching the in the range of Terabits per second.
In order to guarantee such a high data-throughput, the system should be based on a new generation of \gls{soc} together with a \gls{fpga}. 
The \gls{soc} should have high-speed peripherals in order to guarantee the continuous high-speed data-throughput. 
The use with \gls{fpga} should enable flexible adjustment of the system for the user-defined application. 

Furthermore, the system should be compatible with already existing high-speed \gls{daq} frameworks (e.g. based on \gls{pcie} or GPU clusters) and be easily integrable into the system of the user application.
However, stand-alone application should also be possible.
Furthermore, the \gls{daq} should be designed in such way that enables usage also without the time-stretch method.
...
After that the general architecture and basic theory of a photonic time-stretch \gls{daq} is explained.
...
Chapter 2 covers the general architecture of \gls{theresa}, including also state of the art readout-systems, especially the \gls{kapture} which is in operation at the \gls{kara}.
Chapter 3 describes the design steps of the front-end sampling card of \gls{theresa} in detail.
Chapter 4  covers the description of the back-end readout card, as well as the design of the appropriate software.
...
The range of \gls{sr} reaches from hard X-rays down to the infrared region of the electromagnetic spectrum (see \autoref{fig:spectrum}). \gls{sr} shows properties like high intensity, high collimation, polarisation and generation in pulses of well-defined.
...
The photonic time-stretch \gls{daq}, which is developed in this thesis, should also be integrated into the beam diagnostics system at \gls{kara}. 
Therefore, a short overview of some parameters of this facility is given.
...
One scientific focus at \gls{kara} lies in the study of so-called  ``michro-bunching instabilities'' which are described in the following.
...
The emission
Due to this \gls{csr} the bunches interact with their own radiation (see \autoref{fig:microBunch}).
\begin{figure}[tbh]
	\centering
...
This results in the so called micro-bunching instability, the formation of micro-structures (in the sub-millimeter range) in the longitudinal density profile of the electron bunches.
...
\begin{itemize}
	\item A bolometer which produces the input signal for the feedback loop.
...
\begin{itemize}
	...
	\item Association with the new \gls{fpga}-based system, i.e. \gls{theresa}
	\item Finding adequate feedback, programmed in the FPGA
\end{itemize}
would help in solving the problem and allow the control to succeed also at higher currents (goal: \SI{15}{\milli \ampere}). 

\subsection*{Design Requirements for THERESA}
...
Even an ideal $N$-bit converter has be errors because of the quantization process, which behave like noise.
...
Therefore the figures of merit described in the following paragraph are also called frequency-domain dynamic parameters. 
...
\textit{\gls{sinad}} (also called SNDR or S/N+D) denotes the ratio between the \gls{rms} of the signal amplitude to the mean value of the Root-Sum-Square (RSS) of all other spectral components, including harmonics, but excluding  DC (\SI{0}{\hertz}). 
...
\subsection*{Design requirements for THERESA}
The signals to be detected by the photodetector are expected to be very small. 
According to \cite{bielwaski}, peak-to-peak amplitudes are in the range of \SIrange{10}{20}{\percent} of the laser pulse shape.
For a resolution of this small signal of at least 10 \gls{adc} bins, this would require an \gls{enob} of at least 7 bits. 
Small amplitudes of the signal also imply that the noise and distortion of the data converters should be very low, therefore the \gls{sinad} should be high (at least 60dB according to the ideal ADC SNR equation).  
