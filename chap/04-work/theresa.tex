...
\begin{table}[tbh]
	...
	\begin{tabularx}{\textwidth}{Xl}
		...
		[0.3cm]
		\textbf{Loop filter components for PLL1} &                          \\
		$C_{1}$                                            & \SI{39}{\nano\farad}     \\
		$C_{2}$                                            & \SI{1800}{\nano\farad}   \\
		$R_{2}$                                            & \SI{2.2}{\kilo\ohm}      \\
		[0.3cm]
		\textbf{PLL2 parameters}                 & \textbf{Value}           \\
		...
		[0.3cm]
		\textbf{Loop filter components for PLL2} &                          \\
...
\subsection{\gls{pcb} Substrate Selection and Stackup}\label{ssec:substrate}
...
\subsection{Calculating the Geometrical Parameters of Transmission Lines}
...
The change in impedance/parameters is assumed to be negligible, as the difference in dielectric constans between the two boards is not large (\gls{kapture}: $\epsilon_r$ =  3.52, new board: $\epsilon_r$ =  3.61)
...
According to the datasheet of the Megtron6, the dielectric constant $\epsilon_r$ changes over frequency (see \autoref{ssec:substrate}).
As the dielectric constant $\epsilon_r$ of the Megtron6 substrate varies between 3.61 and 3.71 depending on the frequency, the effect of the changing $\epsilon_r$ should also be studied. 
The Si9000e tool provides the possibility to simulate the characteristic impedance versus a changing parameter.
In \autoref{fig:surf_dk} the characteristic impedance $Z_o$ is plotted against $\epsilon_r$.
...
\begin{figure}[tbh]
	...
	\caption[Coplanar waveguide, $Z_o$ vs $\epsilon_r$]{Characteristic impedance $Z_o$ of a coplanar waveguide versus dielectric constant $\epsilon_r$ assuming $a = \SI{213}{\micro \meter}$}
	\label{fig:surf_dk}
\end{figure}
...
\begin{figure}[tbh]
	...
	\caption{$Z_o$ vs. lower trace thickness, assuming $\epsilon_r = 3.61$}
	\label{fig:surf_w1}
\end{figure}

For an estimation of the effect of the upper trace width on the impedance, a constant lower trace width of \SI{213}{\micro \meter} and $\epsilon_r = 3.61$ is assumed, while varying the upper trace width from \SIrange{183}{213}{\micro \meter}.
...
\end{itemize}
For these parameters and an $\epsilon_r = 3.61$ an impedance of \SI{92.35}{\ohm} is calculated with the Si9000e tool.
...
\begin{figure}[tbh]
	...
	\caption{$Z_\text{diff}$ vs. lower trace width $w$, assuming $\epsilon_r = 3.61$}
	\label{fig:surf_diff_w1}
\end{figure}
Setting the width to \SI{155}{\micro \meter} indeed gives an impedance of $Z_\text{diff} = \SI{99.37}{\ohm}$.
The influence of the changing dielectric constant $\epsilon_r$ is studied in this case as well (see \autoref{fig:surf_diff_dk}). 
At the maximal value of $\epsilon_r = 3.71$, the impedance lies around \SI{98.4}{\ohm} corresponding to a change of \SI{0.88}{\percent} compared to the value at $\epsilon_r = 3.61$. 
\begin{figure}[tbh]
	\centering
	\includegraphics[width = \textwidth, height = 0.4\textwidth]{chap/04-work/img/TL/surf_diff/surf_diff_dk}
	\caption{$Z_\text{diff}$ vs. dielectric constant $\epsilon_r$}
	\label{fig:surf_diff_dk}
\end{figure}


Furthermore, assuming $\epsilon_r = 3.61$ and a lower trace width $w = \SI{155}{\micro \meter}$, the impedance over a varying upper trace width is plotted in \autopageref{fig:surf_diff_w2}.


\begin{figure}[tbh]
	\centering
	\includegraphics[width = \textwidth, height = 0.4\textwidth]{chap/04-work/img/TL/surf_diff/surf_diff_w2}
	\caption{$Z_\text{diff}$ vs. upper trace width, assuming lower trace width $w = \SI{155}{\micro \meter}$ and$\epsilon_r = 3.61$}
...
The impedance of this waveguide type depends on the trace width $w$, the trace separation $s$, the trace-to-ground separation $d$, the thickness $t$ of the trace, as well as the thickness of the dielectrics $h_1$ and $h_2$ and their respective dielectric constant $\epsilon_1$ and $\epsilon_2$.
...
\end{itemize}
Thickness of the dielectrics is $h_1 = h_2 = \SI{150}{\micro \meter}$  and the dielectric constant is equal for both ($\epsilon_1 = \epsilon_2 = \epsilon_r = 3.61$.)
...
\begin{figure}[tbh]
	...
	\caption{$Z_\text{diff}$ vs. lower trace width $w$, assuming upper trace width equals to $w$}
...
\begin{figure}[tbh]
	...
	\caption{$Z_\text{diff}$ vs. trace separation $s$, assuming trace width $w = \SI{88}{\micro \meter}$}
	\label{fig:off_diff_s1}
\end{figure}

The influence of the dielectric constant $\epsilon_r$ is shown in \autoref{fig:off_diff_dk}. 
$Z_\text{diff}$ decreases with higher value of $\epsilon_r$ and even get below \SI{90}{\ohm}, exceeding the \SI{10}{\percent} tolerance.
...
\begin{figure}[tbh]
	...
	\caption{$Z_\text{diff}$ vs. dielectric constant $\epsilon_r$, assuming lower trace width $w = \SI{88}{\micro \meter}$ and $\epsilon_r = 3.61$}
...
\begin{figure}[tbh]
	...
	\caption{$Z_\text{diff}$ vs. upper trace width, assuming lower trace width $w = \SI{88}{\micro \meter}$ and$\epsilon_r = 3.61$}
...
\item Components, especially sensitive ones, have to be placed in dedicated ``island''.
...
During routing, take care of the high frequency, sensitive signals first. This guarantees on the one hand the shortest possible connection for these signals. On the other hand this defines the routing for the slower, less critical signals. In order to minimize cross-talk, these signals need to be routed around the high-frequency lines and must not cross these (in the layer directly above or below).
