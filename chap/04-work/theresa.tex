...
Due to its high operating frequency range, the 1.85 mm series is considered a mm-Wave RF connector and finds its use in precision instrumentation and other laboratory applications. The 1.85 mm series has compatible interfacing with the 2.4 mm RF connector.
...
Time skew between the \gls{pll} outputs lies in the range of \SI{30}{\pico \second} \cite{lmk04808b}. 
...
As one fan-out buffer has eight outputs, two chips are needed to cover all 16 sampling channels. 
...
In order to have the opportunity to enable the \gls{spi} mode in later usage, a jumper is placed at this pin so that the pin can be connected to ground ($\eqdev$ logic '0', enabling \gls{spi} mode).
The \texttt{SCLK}, \texttt{SDI} and \texttt{SEN} pins are therefore connected to the \gls{fmc}+ connector.
...
Due to the asymmetric position of the connector on the board, if the \gls{tha} component outpus would be connected direclty to the connector, the signal paths would vary significantly between components.
...
%
%Filtering components need to be considered as well.
%To minimize \gls{emi} on the power supply rails, coming from the component noise on the \gls{fpga}, appropriate filters needs to be placed at the power supplies.
%In order to guarantee stable voltage levels at the power supply pins decoupling/bypassing capacitors need to be placed next to the components. 
%These components need to be placed as close as possible to the power supply pins of the respective components, in order to minimize trace length and therefore parasitic inductance.
